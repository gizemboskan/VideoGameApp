...
class HomePageViewController: UIViewController {
    ...
    var filteredVideoGames = [VideoGameModel]()
    ...
    var dataSource = [VideoGameModel]() // All video games
    var pageSource = [VideoGameModel]() // Page controller games
    var listSource = [VideoGameModel]() // Collection games
    ...
    private func getVideoGames(_ pageNumber: Int) {
        VideoGameClient.getGames(page: pageNumber) { result, error in
            DispatchQueue.main.async {
                ...
                let mappedResult = result.map({ VideoGameModel(id: $0.id, rating: $0.rating, released: $0.released, metacritic: $0.metacritic, name: $0.name, backgroundImage: $0.backgroundImage, isFav: self.favVideoGameIds.contains($0.id))
                })
                if let secondTab = (self.tabBarController?.viewControllers?[1]),
                   let navVC = secondTab as? UINavigationController,
                   let favVC = navVC.viewControllers[0] as? FavoritesViewController {
...
extension HomePageViewController:  UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "GameCell", for: indexPath) as! VideoGameCollectionCell
...
extension HomePageViewController: UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        isFiltering = true
        filteredVideoGames = dataSource.filter({ (game: VideoGameModel) -> Bool in
